...
extension NSDate {
    ...
    // Récupérer date du dernier samedi si on est samedi -> date du jour
    
    
    
    func lastSaturdayDate()->String {
        ...
        let myComponents = myCalendar.components(.CalendarUnitWeekday, fromDate: NSDate())
        ...
        if (weekDay == 7){
            ...
            let lastWeekDateString = styler.stringFromDate(NSDate())
            ...
            var date =  NSDate().dateBySubtractingDays(weekDay)
            let lastSaturdayDateString = styler.stringFromDate(date)

            println("week day \(lastSaturdayDateString)")

            return lastSaturdayDateString
        /// 6 -> -7 , 5 -> -6
        }
    ...
    func lastWeekdayToString() -> String {
