...
class AccueilVC: UITableViewController, UICollectionViewDelegate,UICollectionViewDataSource, MZFormSheetBackgroundWindowDelegate{
    var _melody : Melody = Melody.sharedInstance

    @IBOutlet weak var _pageControl: UIPageControl!
    @IBOutlet var _slideShowView: JBKenBurnsView!
    @IBOutlet weak var _collectionView: UICollectionView!
    var _isPresentingFormSheet : Bool = false

   var _images  = [UIImage(named: "musee2")!,UIImage(named: "musee2")!,UIImage(named: "musee2")!,UIImage(named: "musee2")!,UIImage(named: "musee2")!]
    
    var _pageIndex : Int = Int()
    var _countImage : Int = 0
    var _getLastActuRequestUrl : NSURL?
    ...
    override func viewDidLoad() {
        ...
        for (index, obj) in enumerate(_images)
        {
            ...
            if let checkedUrl = NSURL(string:  _melody._slideShowImagesUrl + "1024_" + String(index+1) + ".png") {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        self._getLastActuRequestUrl = NSURL(string: "http://www.melody.tv/json-new/actualites/appligetall/12")
    ...
    func startGetLastActuRequest(){
        ...
        request(.GET, _getLastActuRequestUrl!)
    ...
    func startGetSeeAlsoRequest(){
       ...
       // self._melody.displayLoadingView(self.view)
         ...
         _melody._seeAlsoUrl = NSURL(string: _melody._seeAlsoUrl.absoluteString! + "?token=" +  _melody._userToken)!
        println("see also url \(_melody._seeAlsoUrl)")
        println("Token \(_melody._userToken)")

        request(.GET, _melody._seeAlsoUrl)
            .responseSwiftyJSON{(request, response, json, error) in
                if (error?.code > -2000)
                {
                    ...
                    //self.getActuItems(json)
                    //rself._collectionView.reloadData()
            ...
            dispatch_async(dispatch_get_main_queue()) {
                ...
                if(data != nil){
                    ...
                    if(self._countImage == self._images.count){
                        ...
                        self._slideShowView.animateWithImages(self._images, transitionDuration: 8, initialDelay: 0, loop: true, isLandscape: true)
                        self._countImage = 0
                    }
                }
